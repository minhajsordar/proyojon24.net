...
const getAllServiceProviders = expressAsyncHandler(async (req, res) => {
    ...
    const serviceProviders = await ServiceProvider.find({ ...keyword }).select('-user -dataUpdatedHistory -dataUpdatedBy -dataCollector -description -phoneNumber -service -serviceCategory -extraCources -degree -serviceList')
    // res.set('Access-Control-Allow-Origin', 'http://localhost:9000');
