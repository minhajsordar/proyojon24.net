<template>
  ...
  <tbody>
    <tr v-for="(district,index) in districtList" :key="index" :class="{'bg-blue-1':index%2 !=0 }">
       ...
       <q-btn class="q-ml-xs" :label="$t('delete')" size="sm" dense color="negative" @click="confirm(district.name[languageStore.language])"/>
        </td>
      </tr>
  </tbody>
  </q-markup-table>
