<template>
  <q-dialog
    v-model="ourBusinessPartners.openBusinessPartnersUpdateDialog"
    persistent
              ...
              <div class="row">
                ...
                <div class="col-lg-8 col-md-7 col-sm-12 col-12">
                  <q-input
                    ref="nameBnEl"
                    v-model="
                      ourBusinessPartners.ourBusinessPartnersInfo.subTitle
                    "
                    outlined
                    dense
                    :rules="[required]"
...
const updateBusinessPartner = () => {
  if (ourBusinessPartners.ourBusinessPartnerImage) {
    nameEnEl.value.validate();
    if (imageEl.value.hasError) {
      return;
    }
  }
  nameEnEl.value.validate();
  nameBnEl.value.validate();
  if (nameEnEl.value.hasError || nameBnEl.value.hasError) {
    return;
  }
  ourBusinessPartners.updateOurBusinessPartners();
};
</script>
